...
function PaginationWithLinksInner({
  ...
  const navToPageSize = useCallback(
    ...
    [pageSizeSelectOptions?.pageSizeSearchParam, searchParams, pageSearchParam, pathname, navigationMode, router]
  );
...
function SelectRowsPerPage({
  ...
  return (
    <div className="flex items-center gap-4">
      <span className="whitespace-nowrap text-sm">Էջի չափը</span>
      <Select value={String(pageSize)} onValueChange={(value) => setPageSize(Number(value))}>
        <SelectTrigger>
          <SelectValue placeholder="Ընտրել էջի չափը">
            {String(pageSize)}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
